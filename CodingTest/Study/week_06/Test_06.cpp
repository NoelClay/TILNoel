...
// 
// 두번째시도
// 위의 가정은 단편적인 모습만 봤지 무수히 많은 반례가 생기는 문제가 있었다.
// 양쪽이 같은 길이일때는 정중앙을 지나가기 때문에 한변의 길이만큼만 못쓰게 된다.
// 문제는 양쪽이 다른 길이일때
// 1. 패턴이 반복되는 경우가 있다.작은 패턴이 반복된다. 맨끝 꼭지점에서 다른 쪽 끝 꼭지점이 연결되는 것이 존재하는 경우가 있다.
// 8 12 는 총 4개로 나뉘어져 2 3 사각형이 4개 반복된다. 2 3 사각형은 대각선이 총 4개를 차지한다.
// 5 15 는 총 5개로 나뉘어지고 1 3 사각형이다. 대각선이 전부 차지하여 3개를 차지한다.
// 10 12는 총 2개로 나뉘어지고 5 6 사각형이다. 세로로 2개를 지나가고 다음에도 계속 2개씩 따라서 총 10개
// 7 10은 총 1개로 나뉘어지고 7 10 사각형이다. 세로로 2개를 지나가다가 중간에 3개 지나가는 경우도 있다.
// - 추가로 든 생각 -
// 무조건 긴쪽을 세로라고 생각을 가정하고 그려보면 가로로 1센치 이동하는 동안 세로는 1센치 이상을 이동하게 되어 있다.
// 세로/가로 만큼 1센치당 세로로 이동한다. 10/7 + 10/7 +... 이걸로 구현을 어떻게 해야될지..
// -미해결..-
//
#if 0
